...
### Step 2: Identify Competitors & Run Automated Scraper
...
Once the primary competitors are identified, determine their main website URLs. 
For each URL, execute the built-in Playwright scraper:
`python .agents/scripts/market_research/scrape_competitor.py [URL]`

Read the JSON output from `stdout` for each competitor to gather high-fidelity, live data on their pricing, features, and hero positioning before generating the analysis.

### Step 3: Analyze Each Competitor with Live Data
